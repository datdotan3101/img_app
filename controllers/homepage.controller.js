...
const homePageController = {
  ...
  checkIfImageSavedByUser: async (req, res) => {
    try {
      ...
      const isSaved = await homepageService.checkIfImageSavedByUser(userId, imageId); // Gọi service kiểm tra
  
      res.json({
        status: 'success',
        data: { isSaved }  // Trả về thông tin đã lưu hay chưa
      });
    } catch (error) {
  ...
  deleteImage: async (req, res) => {
    try {
      const result = await homepageService.deleteImage(req); // Gọi service để xóa
      const resData = responseSuccess(result); // Thành công
      res.json(resData);
    } catch (error) {
  ...
  getCommentsByImageId: async (req, res) => {
    try {
      ...
      const comments = await homepageService.getCommentsByImageId(imageId); // Gọi service để lấy danh sách bình luận
      res.json({
        status: "success",
        data: comments,
      });
